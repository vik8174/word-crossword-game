...
Two things move: the change from one screen of a room to the next ([issue #93](https://github.com/vik8174/word-crossword-game/issues/93)), and the petals falling behind every screen but the game ([issue #103](https://github.com/vik8174/word-crossword-game/issues/103)). What may move and what may not is decided in [ADR 0030](decisions/0030-where-movement-is-allowed.md), and every rule in it costs a step here.
...
6. **The garden coming back, once, at the end of the game.** At **Always** step 8, as the last word goes in. Petals return over the finished board — thickly for a moment, thinning back to the ordinary background over the next few seconds — and the board stays where it is, filled in. It is a garden waking rather than a firework, and nothing else on the screen moves. Then the half that is easy to skip and is the reason the step exists: **press F5 on that finished room.** The result comes back calm, with the ordinary background and no return at all, because the greeting belongs to the change from a game to a finished game and not to the screen being on show — a room that has finished stays finished forever, so a greeting keyed on the screen would play again on every reload and in every tab opened an hour later. Open the same room in a third tab to see it once more: calm, again. Then the second room, at **Always** step 10: a game a player ended early gets neither the greeting nor the petals, and the background of that screen is as still as the game's was.

7. **A tab nobody is looking at is not drawing a garden.** During **Always** step 8, on the guest's profile. Open DevTools > Performance monitor beside the tab, then switch to another tab for half a minute and come back: the frame rate goes to nothing while the tab is behind another one, rather than carrying on at sixty. That is a guest who opened the invite link and went to make tea, and the cost of getting it wrong is their phone warm in their pocket for the length of a game. The price of getting it right is knowingly accepted and is worth recognising rather than reporting: somebody who leaves the tab and comes back after the game has ended finds a static result, the greeting of step 6 having been asleep at the moment it would have played.

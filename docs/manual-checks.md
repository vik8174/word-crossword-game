...
4. **A console that is not red, and one cookie of each name.** DevTools > Console: not a line about a cookie rejected for an invalid domain. Then Storage > Cookies: exactly one `_ga` and exactly one `_ga_<measurement id>`, both on the stage host — two of a name would be a second, host-only cookie written beside the old one. Reload the page: `cid` in the next `collect` is the one it was, so the client id survived.

...
**The landing page left the garden entirely in [#151](https://github.com/vik8174/word-crossword-game/issues/151), and [#152](https://github.com/vik8174/word-crossword-game/issues/152) took the rest of the painted world with it.** "Petals everywhere except the game" below still holds exactly as written — it was never a decision about a canvas, only about which screens the weather falls behind — but the place the petals fall over is three photographs now (the gate, the doors, the hall) rather than a world painted stroke by stroke with a camera flying through it. [ADR 0031](0031-one-camera-and-what-it-promises.md) is the record that described that camera, and it is rewritten to describe what replaced it. Nothing below names the camera at all, because nothing here was ever a decision about it: the camera was one way of putting a screen in front of a picture, and this record is about which screens get weather and which get silence, a question the camera's removal does not touch.

**What still moves, after #152:** the change from one screen of a room to the next ([#93](https://github.com/vik8174/word-crossword-game/issues/93)), the petals below, and the greeting cloth ([0031](0031-one-camera-and-what-it-promises.md)). `prefers-reduced-motion` governs all three, and only three — down from four before this ticket, the camera having been the fourth (`theme.ts`). Which picture is standing behind the app is no longer one of them: swapping the gate for the doors is an attribute changing on an `<img>`, not a `requestAnimationFrame` loop, so there is nothing there for reduced motion or a hidden tab to switch off. A transition between two pictures is [#153](https://github.com/vik8174/word-crossword-game/issues/153)'s to build, and when it exists it will be the fourth thing this record's rules apply to.

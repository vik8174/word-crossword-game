...
- Two earlier records name this constant by its former name and keep it: the note added to [ADR 0017](0017-desktop-first-while-the-grid-outgrows-a-phone.md) when #31 closed, and the addendum to [ADR 0009](0009-room-document-schema.md). Neither is edited, because an ADR says what was decided at the time — the convention 0009 and [0014](0014-telemetry-without-room-ids.md) already follow — so the pointer is corrected here instead. Nothing about their reasoning moves: room size and grid width were never the same question, and the room schema is unchanged
- Nothing a player can see changes with this record. It renames one constant, retires the ceiling language in four comments — one of which promised that rewording to an issue since closed — and writes down a boundary the code was already keeping ([issue #66](https://github.com/vik8174/word-crossword-game/issues/66))

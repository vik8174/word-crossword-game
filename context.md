...
- **Petals** — the weather over the place: they fall over every screen except the one a game is played on. Their absence during a game is deliberate, and their return at the end is the greeting ([0030](docs/decisions/0030-where-movement-is-allowed.md)). They do not fall indoors, and that is geometry rather than a setting — a petal whose place falls inside the temple's doorway is not drawn.
...
- **Location** — a point of the world and a magnification, and one of exactly four: the **gate** (`home`, `create`, `join`), the **doors** (`lobby`), the **hall** (`playing`, `closed-early`) and **congratulations** (`finished`, the hall again, because a game ends where it was played). `connecting` and `unavailable` have none: they are news rather than journeys, and are shown wherever the garden already stands. In this release the locations change instantly.
- **Opening**, also **the doorway** — the 460 × 290 rectangle of the world that is the open front of the temple, with the hall painted inside it in its own 1440 × 900. Not a picture and not a second screen: the same nesting a camera will later fly through, and the reason the weather stops at it.
- **Band** — the surface the interface stands on: `rgba(6,20,16,0.55)`, the shadow under the canopy, with a vermilion hairline down its inner edge. Where a list of words has a column of its own the band is that column, from the top of the window to the bottom; where it has not, the band is as tall as what stands on it. There are two of them in the hall, one on the left in the lobby, and none at the gate.

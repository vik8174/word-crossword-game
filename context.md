...
- **Face** — a typeface in the role it plays, and there are three. The **display face** (Zen Old Mincho) is the letters of the crossword and the two large levels; the **sign face** (Zen Kaku Gothic New) is lettering rather than text, in capitals held apart by 42% of their own size, and it is what the garden letters its words in — the name over the gates, the name of a step and the one action at the gate are the same sign at two sizes, so the tracking is one value and never two (`SIGN_TRACKING`); the **text face** (Zen Kaku Gothic New again, at 400 and at the semibold) is everything that is read rather than looked at — fields, hints, counters, the labels on buttons, the names of panels and of players. Sign face and text face are the same family and two different jobs, told apart by weight and tracking and never by name; whatever the reader's own system draws is no longer a face of its own but the tail of all three stacks, drawn on the first frame and drawn instead if a file never arrives ([#124](https://github.com/vik8174/word-crossword-game/issues/124)).

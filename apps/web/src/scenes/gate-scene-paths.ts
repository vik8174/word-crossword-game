 ...
 * reads `GATE_AVIF` too — to preload exactly the file `/` draws rather than a
 * second copy of the same path typed out by hand (`build/scene-preload.ts`) —
 * and a build script has no business pulling a component's own dependency
 * tree in just to read one string out of it.
 */

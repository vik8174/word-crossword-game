import { BANK_TREES, FAR_TREES, GROVE_TREES, RIDGE_TREES, THICKET_TREES } from './forest-trees';
...
/**
 * Which step of the stairs this is.
 *
 * A union rather than a `string`, because two of the five names are read back
 * by {@link paintScene} to decide where the ground, the water, the temple and
 * the cherries take their places in the row. A misspelling there would not be
 * an error, it would be a temple that quietly stopped being painted.
 */
export type PlaneName = 'ridge' | 'far' | 'grove' | 'bank' | 'thicket';
 ...
 * and therefore before the thicket in front of it.
 *
 * Named here rather than spelled again in {@link paintScene}, so that the row
 * and the things standing in it are one decision. The ground and the water
 * arrive with the temple, because the ground is what it is built on.
 */
export const TEMPLE_AFTER: PlaneName = 'bank';

/**
 * And where the cherries stand: on the plane in front of the temple, so that
 * the petals fall past the building rather than out of it.
 */
export const SAKURA_ON: PlaneName = 'thicket';

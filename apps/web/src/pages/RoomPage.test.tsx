...
describe('RoomPage', () => {
  describe('arriving at the link', () => {
    ...
    // handed a full room frame to stand in while they waited there: a header, a
    // heading unseen but laid out, three zones. That frame is what a room looks
    // like once it exists, so a guest saw it appear at the gate and then again,
    // moments later, in the doorway the camera had by then arrived at — the same
    // frame twice, with nothing behind it either time (issue #132). Waiting for
    // news of a room that has not answered yet is not a room, so it gets none of
    // its shape.
